# -*- coding: utf-8 -*-
"""
Created on Fri Mar 13 20:06:29 2015
...
##two new facts
#learned how to add average trend line within scatterplot
plt.plot(decade_mean.index, decade_mean.values, 'o-',
        color='r', lw=3, label='Decade Average')
plt.scatter(data.year, data.score, alpha=.04, lw=0, color='k')

#learned how to exclude bad data
data.runtime[data.runtime==0] = np.nan


#Average run time has remained pretty stagnant throughout time
decade_run_mean = data.groupby(decade).runtime.mean()
decade_run_mean.name = 'Decade Mean'
print decade_mean
plt.plot(decade_run_mean.index, decade_run_mean.values, 'o-',
        color='r', lw=3, label='Decade Average')
